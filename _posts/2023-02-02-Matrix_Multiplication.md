...
subtitle: Speeding up sloww matrix operations in python
description: Optimizing slow matrix operations
excerpt: >
    I was asked to speed the inner loop that runs many times inside a Markov Chain Monte 
    Carlo problem. This post is about different approaches to improving performance. 
    Possibly, some insights here are applicable beyond the details of the specific 
    problem.
mathjax: true
header_img: /assets/images/2023/02/02/mikita-yo-OCrl1Tkt630-unsplash.jpg
og_image: /assets/images/2023/02/02/mikita-yo-OCrl1Tkt630-unsplash.jpg
header_type: hero
last_modified_at: 2024-10-31
tags: [blog,python,numpy,coding,maths,matrix,mcmc]
categories: blog
---
...
The slowest part of each iteration is taking the product of the 
[Z-scores](https://en.wikipedia.org/wiki/Standard_score) of a constant matrix, $$X$$, 
and float-valued vectors $$u$$ or $$v$$. Here, $$X$$ is a tall $$n \times m$$ matrix 
with $$n=45000$$ and $$m=3$$ with all elements in $$X$$ are $$-1$$, $$0$$ or $$1$$. 
The vectors $$u$$ and $$v$$ have lengths $$m$$ (long) and $$n$$ (short) respectively.
...
notebook in [python (v3.10.4)](https://www.python.org) on my M1-pro MacBook Pro. 
Additionally, the [numpy (v1.22.4)](https://numpy.org) and 
[scipy (v1.8.1)](https://scipy.org) packages were installed. 
...
That's half the speed of `calc_rearranged`. Building on the split method, one can avoid 
...
There are many 0s in the matrix, perhaps this problem can be approach using 
...
Sparse matrices improve all the tested methods w.r.t. the baseline. Still, the index 
