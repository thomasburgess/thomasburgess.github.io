...
def print_pixels(pixels, blank=".", full="█"):
    min0, max0, min1, max1 = limits(pixels)
    print(min0, max0, min1, max1)
    for y in range(min1, max1+1):
        print("".join((blank, full)[pixels.get((x, y), False)] for x in range(min0, max0+1)))
```
The last function just prints the map to stdout, for the example data:
...
For part 2 my output has a hole in the floor where no sand runs...
