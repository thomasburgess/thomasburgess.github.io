...
mathjax: false
og_image: /assets/images/2022/12/01/aoc2022.png
header_type: hero
categories: [blog,advent,code]
header_img: /assets/images/2022/12/01/aoc2022.png
description: Advent of code 2022
excerpt: >
    I will do this years Advent of Code 2022 in vanilla (only included libraries) 
    python 3.11. I've joined the challenge a few times before - but never finished. 
    Let's see how many days I manage this round :D
project_links:
    - url: https://github.com/thomasburgess/advent2022
      icon: fab fa-github
...
I will do this years [Advent of Code 2022](https://adventofcode.com/2022) in vanilla 
...
* _Update 1: the answer is 16, December is such a busy month :)_
* _Update 2: [see my post on AoC 2023]({% post_url 2023-12-01-advent2023 %})_ 
...
different parts. Here is the pathon template I use to get started:
...
I'll keep my code on this 
[github repository](https://github.com/thomasburgess/advent2022), most of the code will 
appear in this blog post.
...
* It's easy and useful to print unicode characters `sand="🟨"`
...
Data: rows with games, the first column is the opponent move, and the second is the 
player move.
...
Score: Player move (1 for rock, 2 for paper, and 3 for scissors), plus 0 for loss, 3 for
draw and 6 for win.
...
Part 1 is solved by summing the set intersections of the first and second part of the 
sacks:
```python
...
The data is split into two parts separated by a blank line. In the first part, stacks of
items are listed in columns, with column index on the last row (this index can be
ignored). Items are on the form `[X]` where `X` is a single character item id. In the 
...
Solution: generate the length of the `set` of n character spans until it equals n. For
this [`itertools.takewhile`](https://docs.python.org/3.8/library/itertools.html#itertools.takewhile)
is very useful.
...
```
Plugging in `n=4` into this gives the correct solution for part 1, and part 2 likewise 
with `n=14`.
...
A file system is explored using `cd` and `ls`. The first `cd` is always to `/` and `ls`
are always in current directory. Lines with commands starts with `$`. Listings of
directories start with `dir`, and listings of file starts with file size.
...
The code is read into a tree strucutre of nested dictionaries using `read_tree`. For
files the value is the filesize int instead of a dictionary. I based the thinking here
on my [Tree Hack blog post](https://thomasburgess.github.io/blog/2021/07/18/tree_hack.html).
...
Task 2: Find smallest directory that when removed leaves 30000000 free storage of a 
total of 70000000.
...
Here, for each tree the number of consecutive shorter trees is counted and multiplied 
together for each direction. 

This task took quite some time to debug, I think I didn't parse the instructions
correctly...
...
The task is to move the head of a rope according to the moves, and track how the tail
trails.
...
The solution for part2 also solves part1 so I refactored simualte accordingly. As I had
...
The printer gives this expected output for the tail visits for the longer example in 
part 2.
```
#.....................
#.............###.....
...
Next, the program is executed using `run_program`to track `cycle` count and `x` after
each instructions. `x` starts at 1.
...
I did not have the time to go on this year, maybe I will have better luck next year.
